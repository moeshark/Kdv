% calculates RMSE for different periodicities
% input: non-linearity coefficient and length L

function r = RMSE2(xi,L)
    
    % FFT Parameters
    ModelParams.N = 64;  %size of time series data N
    ModelParams.xi = xi;  % non-linearity coefficient
    ModelParams.dt = .4/ModelParams.N^2; %% time step
    ModelParams.M = 90000; % number of time steps to generate
    ...
    rmse_mat = zeros(length(L),resparams.predict_length);% matrix of RMSE Vals

    % train the reservoir each time
    for n = 1:length(L)
        ModelParams.P = 2*L(n)*pi;
